...
slate.utils = {
  ...
  getParameterByName: function(name, url) {
    ...
    name = name.replace(/[[\]]/g, '\\$jq&');
    var regex = new RegExp('[?&]' + name + '(=([^&#]*)|&|#|$jq)'),
...
theme.Images = (function() {
  ...
  function getSizedImageUrl(src, size) {
    ...
    var match = src.match(
      /\.(jpg|jpeg|gif|png|bmp|bitmap|tiff|tif)(\?v=\d+)?$jq/i
    );
...
theme.Currency = (function() {
  var moneyFormat = '$jq{{amount}}'; // eslint-disable-line camelcase
...
theme.ProductVideo = (function() {
  ...
  function hostFromVideoElement(video) {
    ...
    if (video.tagName === 'IFRAME') {
      if (
        /^(https?:\/\/)?(www\.)?(youtube\.com|youtube-nocookie\.com|youtu\.?be)\/.+$jq/.test(
          video.src
        )
      ) {
  ...
  Cart.prototype = Object.assign({}, Cart.prototype, {
    ...
    _liveRegionText: function(item) {
      ...
      var liveRegionText =
        ...
        ': [QuantityLabel]: [Quantity], [Regular] [$jq$jq] [DiscountedPrice] [$jq]. [PriceInformation]';
      ...
      liveRegionText = liveRegionText
        ...
        .replace('[$jq$jq]', regularPrice)
        .replace('[DiscountedPrice]', discountLabel)
        .replace('[$jq]', discountPrice)
...
theme.Product = (function() {
  function Product(container) {
    var $jqcontainer = (this.$jqcontainer = $jq(container));
    var sectionId = $jqcontainer.attr('data-section-id');
    this.ajaxEnabled = $jqcontainer.data('ajax-enabled');
    ...
    this.settings = {
      ...
      bpSmall: false,
      enableHistoryState: $jqcontainer.data('enable-history-state') || false,
    ...
    this.$jqquantityInput = $jq(this.selectors.quantity, $jqcontainer);
    this.$jqerrorMessageWrapper = $jq(
      this.selectors.errorMessageWrapper,
      $jqcontainer
    );
    this.$jqaddToCart = $jq(this.selectors.addToCart, $jqcontainer);
    this.$jqaddToCartText = $jq(this.selectors.addToCartText, this.$jqaddToCart);
    this.$jqloader = $jq(this.selectors.loader, this.$jqaddToCart);
    this.$jqloaderStatus = $jq(this.selectors.loaderStatus, $jqcontainer);
    this.$jqshopifyPaymentButton = $jq(
    ...
    if (!$jq('#ProductJson-' + sectionId).html()) {
      return;
    }
    ...
    this.settings.zoomEnabled = $jq(this.selectors.productImageWraps).hasClass(
      'js-zoom-enabled'
    );
  ...
  Product.prototype = _.assignIn({}, Product.prototype, {
    _stringOverrides: function() {
      ...
      $jq.extend(theme.strings, theme.productStrings);
    },
...
//           if ($jq(self.selectors.productThumbImages).length > 3) {
//             self._initThumbnailSlider();
//           }

//           // destroy image zooming if enabled
//           if (self.settings.zoomEnabled) {
//             $jq(self.selectors.productImageWraps).each(function() {
//               _destroyZoom(this);
//             });
//           }
...
//     },
    ...
    _initVariants: function() {
      var options = {
        $jqcontainer: this.$jqcontainer,
        enableHistoryState:
          this.$jqcontainer.data('enable-history-state') || false,
    ...
    _initImageSwitch: function() {
      if (!$jq(this.selectors.productThumbImages).length) {
        return;
      }

      var self = this;

      $jq(this.selectors.productThumbImages)
        .on('click', function(evt) {
          evt.preventDefault();
          var $jqel = $jq(this);

          var imageId = $jqel.data('thumbnail-id');
    ...
    _initAddToCart: function() {
      $jq(this.selectors.productForm, this.$jqcontainer).on(
        'submit',
        function(evt) {
          evt.preventDefault();

          if (this.$jqaddToCart.is('[aria-disabled]')) {
            return;
          }

          this.$jqpreviouslyFocusedElement = $jq(':focus');

          var isInvalidQuantity = this.$jqquantityInput.val() <= 0;
          ...
          if (!isInvalidQuantity && this.ajaxEnabled) {
            ...
            var $jqdata = $jq(this.selectors.productForm, this.$jqcontainer);
            this._addItemToCart($jqdata);
            return;
          }
    ...
    _addItemToCart: function(data) {
      ...
      $jq.post(params)
        ...
        .fail(
          function(response) {
            this.$jqpreviouslyFocusedElement.focus();
    ...
    _handleButtonLoadingState: function(isLoading) {
      if (isLoading) {
        this.$jqaddToCart.attr('aria-disabled', true);
        this.$jqaddToCartText.addClass(this.classes.hidden);
        this.$jqloader.removeClass(this.classes.hidden);
        this.$jqshopifyPaymentButton.attr('disabled', true);
        this.$jqloaderStatus.attr('aria-hidden', false);
      } else {
        this.$jqaddToCart.removeAttr('aria-disabled');
        this.$jqaddToCartText.removeClass(this.classes.hidden);
        this.$jqloader.addClass(this.classes.hidden);
        this.$jqshopifyPaymentButton.removeAttr('disabled');
        this.$jqloaderStatus.attr('aria-hidden', true);
      }
    },

    _showErrorMessage: function(errorMessage) {
      $jq(this.selectors.errorMessage, this.$jqcontainer).html(errorMessage);

      if (this.$jqquantityInput.length !== 0) {
        this.$jqquantityInput.addClass(this.classes.inputError);
      }

      this.$jqerrorMessageWrapper
    ...
    _hideErrorMessage: function() {
      this.$jqerrorMessageWrapper.addClass(
        this.classes.productFormErrorMessageWrapperHidden
      );

      if (this.$jqquantityInput.length !== 0) {
        this.$jqquantityInput.removeClass(this.classes.inputError);
      }
    },

    _setupCartPopup: function(item) {
      this.$jqcartPopup = this.$jqcartPopup || $jq(this.selectors.cartPopup);
      this.$jqcartPopupWrapper =
        this.$jqcartPopupWrapper || $jq(this.selectors.cartPopupWrapper);
      this.$jqcartPopupTitle =
        this.$jqcartPopupTitle || $jq(this.selectors.cartPopupTitle);
      this.$jqcartPopupQuantity =
        this.$jqcartPopupQuantity || $jq(this.selectors.cartPopupQuantity);
      this.$jqcartPopupQuantityLabel =
        this.$jqcartPopupQuantityLabel ||
        $jq(this.selectors.cartPopupQuantityLabel);
      this.$jqcartPopupClose =
        this.$jqcartPopupClose || $jq(this.selectors.cartPopupClose);
      this.$jqcartPopupDismiss =
        this.cartPopupDismiss || $jq(this.selectors.cartPopupDismiss);
      this.$jqcartPopupImagePlaceholder =
        this.$jqcartPopupImagePlaceholder ||
        $jq(this.selectors.cartPopupImagePlaceholder);
    ...
    _updateCartPopupContent: function(item) {
      var quantity = this.$jqquantityInput.length ? this.$jqquantityInput.val() : 1;

      this.$jqcartPopupTitle.text(item.product_title);
      this.$jqcartPopupQuantity.text(quantity);
      this.$jqcartPopupQuantityLabel.text(
    ...
    _setupCartPopupEventListeners: function() {
      ...
      this.$jqcartPopupClose.on('click', this._hideCartPopup.bind(this));
      this.$jqcartPopupDismiss.on('click', this._hideCartPopup.bind(this));
      $jq('body').on('click', this._onBodyClick.bind(this));
    },

    _setCartPopupPlaceholder: function(imageUrl, imageAspectRatio) {
      this.$jqcartPopupImageWrapper =
        this.$jqcartPopupImageWrapper || $jq(this.selectors.cartPopupImageWrapper);

      if (imageUrl === null) {
        this.$jqcartPopupImageWrapper.addClass(this.classes.hidden);
        return;
      }

      var $jqplaceholder = $jq(this.selectors.cartPopupPlaceholderSize);
      var maxWidth = 95 * imageAspectRatio;
      var heightRatio = 100 / imageAspectRatio;

      this.$jqcartPopupImagePlaceholder.css('max-width', maxWidth);

      $jqplaceholder.css('padding-top', heightRatio + '%');
    },

    _setCartPopupImage: function(imageUrl, imageAlt) {
      if (imageUrl === null) return;

      this.$jqcartPopupImageWrapper.removeClass(this.classes.hidden);
      ...
      image.onload = function() {
        this.$jqcartPopupImagePlaceholder.addClass(this.classes.hidden);
        this.$jqcartPopupImageWrapper.append(image);
      }.bind(this);
    },
    ...
    _setCartPopupProductDetails: function(
      ...
      this.$jqcartPopupProductDetails =
        this.$jqcartPopupProductDetails ||
        $jq(this.selectors.cartPopupProductDetails);
      ...
      if (variantPropertiesHTML.length === 0) {
        this.$jqcartPopupProductDetails.html('');
        this.$jqcartPopupProductDetails.attr('hidden', '');
      } else {
        this.$jqcartPopupProductDetails.html(variantPropertiesHTML);
        this.$jqcartPopupProductDetails.removeAttr('hidden');
      }
    },
    ...
    _setCartQuantity: function(quantity) {
      this.$jqcartPopupCartQuantity =
        this.$jqcartPopupCartQuantity || $jq(this.selectors.cartPopupCartQuantity);
      var ariaLabel;
      ...
      this.$jqcartPopupCartQuantity.text(quantity).attr('aria-label', ariaLabel);
    },

    _setCartCountBubble: function(quantity) {
      this.$jqcartCountBubble =
        this.$jqcartCountBubble || $jq(this.selectors.cartCountBubble);
      this.$jqcartCount = this.$jqcartCount || $jq(this.selectors.cartCount);

      this.$jqcartCountBubble.removeClass(this.classes.hidden);
      this.$jqcartCount.text(quantity);
    },
    ...
    _showCartPopup: function() {
      ...
      slate.a11y.trapFocus({
        $jqcontainer: this.$jqcartPopupWrapper,
        $jqelementToFocus: this.$jqcartPopup,
    ...
    _hideCartPopup: function(event) {
      ...
      $jq(this.selectors.cartPopupImage).remove();
      this.$jqcartPopupImagePlaceholder.removeClass(this.classes.hidden);
      ...
      if (setFocus) this.$jqpreviouslyFocusedElement[0].focus();

      this.$jqcartPopupWrapper.off('keyup');
      this.$jqcartPopupClose.off('click');
      this.$jqcartPopupDismiss.off('click');
      $jq('body').off('click');
    },
    ...
    _onBodyClick: function(event) {
      ...
      if (
        $jqtarget[0] !== this.$jqcartPopupWrapper[0] &&
        !$jqtarget.parents(this.selectors.cartPopup).length
      ) {
        this._hideCartPopup(event);
      }
    },
    ...
    _setActiveThumbnail: function(imageId) {
      ...
      var $jqthumbnailWrappers = $jq(
        this.selectors.productThumbListItem + ':not(.slick-cloned)',
        this.$jqcontainer
      );

      var $jqactiveThumbnail = $jqthumbnailWrappers.find(
      ...
      $jqactiveThumbnail.addClass(this.classes.activeClass);
      $jqactiveThumbnail.attr('aria-current', true);

      if (!$jqthumbnailWrappers.hasClass('slick-slide')) {
        return;
      }

      var slideIndex = $jqactiveThumbnail.parent().data('slick-index');

      $jq(this.selectors.productThumbs).slick('slickGoTo', slideIndex, true);
    },
    ...
    _switchImage: function(imageId) {
      ...
      $jqnewImage.removeClass(this.classes.hidden);
      $jqotherImages.addClass(this.classes.hidden);
    },

    _handleImageFocus: function(evt) {
      if (evt.keyCode !== slate.utils.keyboardKeys.ENTER) return;

      $jq(this.selectors.productFeaturedImage + ':visible').focus();
    },
    ...
    _initThumbnailSlider: function() {
      ...
      $jq(this.selectors.productThumbs).slick(options);

      // Accessibility concerns not yet fixed in Slick Slider
      $jq(this.selectors.productThumbsWrapper, this.$jqcontainer)
        .find('.slick-list')
    ...
    _destroyThumbnailSlider: function() {
      $jq(this.selectors.productThumbs).slick('unslick');
    ...
    _liveRegionText: function(variant) {
      // Dummy content for live region
      var liveRegionText =
        '[Availability] [Regular] [$jq$jq] [Sale] [$jq]. [UnitPrice] [$jq$jq$jq]';
      ...
      liveRegionText = liveRegionText
        ...
        .replace('[$jq]', salePrice)
        .replace('[UnitPrice]', unitLabel)
        .replace('[$jq$jq$jq]', unitPrice)
        .trim();
    ...
    _updateAddToCart: function(evt) {
      ...
      if (variant) {
        if (variant.available) {
          ...
          $jq(this.selectors.addToCartText, this.$jqcontainer).text(
            theme.strings.addToCart
          );
          this.$jqshopifyPaymentButton.show();
        } else {
          ...
          this.$jqshopifyPaymentButton.hide();
        }
      } else {
        this.$jqaddToCart
          .attr('aria-disabled', true)
    ...
    _updatePrice: function(evt) {
      ...
      var $jqpriceContainer = $jq(this.selectors.priceContainer, this.$jqcontainer);
      var $jqregularPrice = $jq(this.selectors.regularPrice, $jqpriceContainer);
      var $jqsalePrice = $jq(this.selectors.salePrice, $jqpriceContainer);
      var $jqunitPrice = $jq(this.selectors.unitPrice, $jqpriceContainer);
      var $jqunitPriceBaseUnit = $jq(
      ...
      if (variant.compare_at_price > variant.price) {
        ...
        $jqpriceContainer.addClass(this.classes.productOnSale);
      } else {
        // Regular price
        $jqregularPrice.html(
          theme.Currency.formatMoney(variant.price, theme.moneyFormat)
      ...
      if (variant.unit_price) {
        ...
        $jqunitPriceBaseUnit.html(this._getBaseUnit(variant));
        $jqpriceContainer.addClass(this.classes.productUnitAvailable);
      }
    },
    ...
    _updateSKU: function(evt) {
      ...
      // Update the sku
      $jq(this.selectors.SKU).html(variant.sku);
    },

    onUnload: function() {
      this.$jqcontainer.off(this.settings.namespace);
    }
  });

  function _enableZoom(el) {
    var zoomUrl = $jq(el).data('zoom');
    $jq(el).zoom({
      url: zoomUrl
    });
  }
